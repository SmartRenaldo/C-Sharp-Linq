...
namespace _04_LinqJoin
{
    public class HospitalHandler
    {
        ...
        public void ShowPatientsWithHospitalName()
        {
            var newCollection = from patient in PatientList
                                join hospital in HospitalList on patient.HospitalId equals hospital.Id
                                orderby patient.Name
                                select new { PatientName = patient.Name, HospitalName = hospital.Name };

            Console.WriteLine("PatientsWithHospitalName:");

            foreach (var collection in newCollection)
            {
                Console.WriteLine("Patient {0} from Hospital {1}", collection.PatientName, collection.HospitalName);
            }
        }
    }
}
